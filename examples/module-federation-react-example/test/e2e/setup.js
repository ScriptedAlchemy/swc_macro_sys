...
export const test = base.extend({
  ...
  moduleFederation: async ({ page }, use) => {
    ...
    const mf = {
      ...
      async isRemoteEntryLoaded() {
        const response = await page.request.get('http://localhost:3002/remoteEntry.js');
      ...
      async checkSharedDependencies() {
        const sharedLibs = await page.evaluate(() => {
          ...
          const reactVersions = window.__REACT_DEVTOOLS_GLOBAL_HOOK__?.renderers?.size || 0;
  ...
  performanceMonitor: async ({ page }, use) => {
    ...
    const monitor = {
      ...
      getTotalBundleSize() {
        return metrics.resourceSizes.reduce((sum, resource) => sum + resource.size, 0);
      },
